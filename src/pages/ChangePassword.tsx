...
const ChangePassword = () => {
  ...
  const handleSendOtp = async (e?: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE || "http://127.0.0.1:8000"}/auth/forgot-password-send-otp`, {
        method: "POST",
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE || "http://127.0.0.1:8000"}/auth/reset-password`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
